package com.bergburg.bergburg.constantes;

public class Constantes {

   // public static String PEDIDO_FECHADO = "Pedido fechado";
    //public static String PEDIDO_ABERTO = "Pedido aberto";
    public static String ID_MESA = "Id da mesa";
    public static String ID_CATEGORIA = "Id categoria";
    public static String ID_PEDIDO = "Id pedido";
    public static String TITULO_CATEGORIA = "Titulo Categoria";
    public static String MESA = "Mesa";
    public static String STATUS = "Status";
    public static String USUARIO = "Usuario";

    public static String FECHADO = "Fechado";
    public static String ABERTO = "Aberto";

    public static String MESA_ABERTA = "Mesa aberta!";
    public static String NAO_CONSEGUIR_ABRIR_MESA = "Não foi possível abrir mesa! Tente novamente.";
    public static String OCUPADO = "Ocupado";
    public static String LIVRE = "livre";
    public static String VERIFIQUE_CONEXAO = "Verifique sua conexao com a internet!";

    public static String SIM = "SIM";
    public static String NAO = "NAO";
    public static String REMOVIDO = "REMOVIDO";
    public static String NAO_REMOVIDO = "Não removido, tente novamente!";
    public static String ATIVO = "ATIVO";

    public static String ADICIONADO = "Adicionado!";
    public static String NAO_ADICIONADO = "Não foi possível Adicionar!";
    public static String ATUALIZADO = "ATUALIZADO";
    public static String NAO_ATUALIZADO = "Não atualizado tente novamente!";


    public static String ENVIADO      = "Enviado";
    public static String PEDIDO_NAO_ENVIADO      = "Pedido não enviado! Tente novamente.";
    public static String CANCELADO     = "Cancelado";
    public static String NAO_CANCELADO = "Não cancelado! Tente novamente.";
    public static String PREPARANDO    = "Preparando";
    public static String CONCLUIDO        = "Concluido";
    public static String NAO_ENVIADO   = "Não enviado";

    public static String USUARIO_LOGADO   = "Logado";
    public static String USUARIO_DESLOGADO   = "Deslogado";




}
